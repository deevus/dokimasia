...
class Doki:
    ...
    def _artifact_slug(self, artifact_name: str | None) -> str:
        label = artifact_name or f"run-{self._run_count}"
        slug = slugify(label)
        return slug or f"run-{self._run_count}"
...
def doki_factory(request: _pytest.FixtureRequest, tmp_path: Path):
    def factory(
        *,
        ...
        artifact_root = Path(artifact_dir) if artifact_dir is not None else tmp_path / ".doki-artifacts" / _node_slug(request.node.nodeid)
